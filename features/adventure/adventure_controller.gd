...
var _interaction_props: Array[AdventureProp] = []
...
func _find_interaction_target(origin: Vector2, facing: Vector2) -> Node2D:
	...
	for actor: WildCreatureActor in _wild_targets:
		...
		if distance < best_distance:
			...
			best_target = actor
	for prop: AdventureProp in _interaction_props:
		if not is_instance_valid(prop):
			continue
		var distance: float = probe.distance_to(prop.global_position)
		if distance < best_distance:
			best_distance = distance
			best_target = prop
	return best_target
...
func _spawn_authored_world() -> void:
	...
	_spawn_prop(AdventureProp.Kind.NPC, Vector2(875.0, 675.0), "Ranger Sela", "Wild creatures are visible in the preserve now. Approach calmly, press E, weaken one in battle, then cast a Trail Prism.")
...
func _spawn_prop(kind: AdventureProp.Kind, world_position: Vector2, title: String = "", text: String = "") -> AdventureProp:
	...
	if not text.is_empty():
		_interaction_props.append(prop)
	return prop
